...
const QRScan = () => {
  ...
  const verifyPayload = async (rawPayload: string) => {
    ...
    try {
      ...
      if (parsed.type === "hmac") {
        const { data } = await api.post('/profiles/validate-spoc-qr', {
        ...
        // verify-temp-credentials is usually part of activate-account logic or a separate check
        // Looking at backend, activate-account is the main one. 
        // But for verification only:
        toast.info("Temp credentials detected. Proceeding to registration.");
        sessionStorage.setItem("eternia_spoc_verified", "true");
        sessionStorage.setItem("eternia_temp_payload", rawPayload); // Store for registration
        navigate("/register");
      }
    } catch (err: any) {
      const msg = err.response?.data?.error || "Verification failed. Please check your connection and try again.";
      toast.error(msg);
    }
    setIsVerifying(false);
  };
