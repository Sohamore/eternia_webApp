...
const Register = () => {
  ...
  useEffect(() => {
    ...
    if (instId) {
      api.get(`/institutions/${instId}`)
        .then(({ data }) => {
          if (data?.institution?.institution_type) setInstitutionType(data.institution.institution_type);
        })
        .catch(() => {});
    }
  }, []);
  ...
  const handleVerifyStudentId = async () => {
    ...
    try {
      ...
      if (data?.valid) {
        ...
        setStudentIdVerified(true);
        toast.warning("Verification records not available. Proceeding without verification.");
      } else {
  ...
  const handleStep2Submit = async (e: React.FormEvent) => {
    ...
    try {
      if (tempCredentialId) {
        const { data } = await api.post("/auth/activate-account", {
          ...
          emergencyContact: {
            ...
            relation: formData.contactIsSelf ? "Self" : formData.emergencyRelation,
            isSelf: formData.contactIsSelf,
          },
          studentIdData: {
            rawId: formData.studentId,
            idType: isSchool ? "erp" : "apaar",
          }
        });
        ...
        const { error } = await signUp(formData.username.trim(), formData.password, {
