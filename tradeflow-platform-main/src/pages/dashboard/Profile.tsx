...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const loadPrivate = async () => {
      try {
        const { data } = await api.get("/profiles/me/private");
        if (data && data.data) {
          const pd = data.data;
          setContactIsSelf(pd.contact_is_self ?? null);
          setEmergencyName(pd.emergency_name_encrypted || "");
          setEmergencyPhone(pd.emergency_phone_encrypted || "");
          setEmergencyRelation(pd.emergency_relation || "");
          const verified = !!(pd.apaar_verified || pd.erp_verified);
  ...
  const handleSaveProfile = async () => {
    ...
    setIsSaving(true);
    try {
      const updates: { bio: string; specialty?: string } = { bio };
  ...
  const handleVerifyStudentId = async () => {
    ...
    try {
      ...
      const { data: inst } = await api.get(`/admin/institutions`);
      const institutionData = inst?.institutions?.find((i: any) => i.id === institutionId);
      const idType = institutionData?.plan_type === "school" ? "erp" : "apaar";

      const { data } = await api.post("/profiles/verify-student-id", {
        institution_id: institutionId, id_type: idType, raw_id: studentId.trim(), claim_for_user_id: user.id
      });
      ...
      if (data.verified) {
        ...
        setIdVerified(true);
        setStudentId("••••••••");
        toast.success(data.claimed ? "ID verified successfully" : (data.reason === "no_records" ? "Verified (no institution records to check against)" : "Verified"));
      } else {
  ...
  const handleSaveEmergency = async () => {
    ...
    try {
      await api.patch("/profiles/emergency-contact", {
        contact_is_self: contactIsSelf,
        emergency_name: contactIsSelf ? "" : emergencyName.trim(),
