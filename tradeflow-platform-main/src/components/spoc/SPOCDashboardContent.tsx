...
const SPOCDashboardContent = () => {
  ...
  const { data: students = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      // The backend /admin/members endpoint already filters by institution for SPOC role
      return data.members.filter((m: any) => m.role === 'student');
    },
    enabled: !!institutionId,
  });

  const { data: dashboardStats } = useQuery({
    queryKey: ["spoc-dashboard-stats", institutionId],
    queryFn: async () => {
  ...
  const todaySessions = dashboardStats?.activeToday || 0;
  const peerSessionCount = dashboardStats?.peerCount || 0;
  const expertSessionCount = dashboardStats?.appointmentCount || 0;
  ...
  // Polling fallback for escalation notifications
  useEffect(() => {
    if (!user) return;
  ...
  const { data: flaggedEntries = [] } = useQuery({
    queryKey: ["spoc-flagged", institutionId],
    queryFn: async () => {
  ...
  const { data: reportData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get(`/admin/reports?days=${reportDateFilter}`);
      setReportLastUpdated(new Date());
      return data.reports || { appointments: 0, peerSessions: 0, moodEntries: 0, questCompletions: 0 };
    },
    enabled: studentIds.length > 0,
  });

  // Removed realtime refresh for reports as REST doesn't support Supabase channels.
  // Realtime is typically overkill for admin analytics anyway. Polling could be added if needed.

  const { data: stabilityPoolBalance = 0 } = useQuery({
    queryKey: ["stability-pool", institutionId],
    queryFn: async () => {
      if (!institutionId) return 0;
      const { data } = await api.get('/admin/credits/pool');
  ...
  const createEscalation = useMutation({
    mutationFn: async () => {
      ...
      await api.post('/admin/escalations', {
  ...
  const { data: tempCredStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get(`/admin/temp-credentials?institution_id=${institutionId}`);
  ...
  const grantCreditsToStudents = async () => {
    ...
    try {
      ...
      const { data } = await api.post('/admin/credits/grant-bulk', {
        institution_id: institutionId,
        amount,
        notes: "SPOC bulk grant"
      });
      
      toast.success(`Granted ${amount} ECC to ${data.count} students`);
    } catch (err: any) {
      toast.error(err.response?.data?.error || err.message || "Failed to grant credits");
    }
    setIsGranting(false);
  };
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                ...
                                const { data } = await api.post('/admin/escalations/emergency-contact', {
                                  student_id: snippet.student_id || snippet.student_username,
                                });
                                
                                if (data?.contact) {
                                  // Since we don't have a direct endpoint to update the snippet only, 
                                  // we will just show it in a toast for now or trigger a refetch if we had an endpoint
                                  toast.success(`Contact: ${data.contact.name} (${data.contact.phone})`);
                                  // We would normally save this back to the escalation_request trigger_snippet,
                                  // but our /admin/escalations doesn't have an update endpoint for arbitrary fields.
                                } else {
                                  toast.info("No emergency contact on file for this student");
                                }
                              } catch (err: any) {
                                toast.error(err.response?.data?.error || err.message || "Failed to fetch contact");
                              }
                            }}
                          >
