...
const AvatarUpload = ({ size = "lg", institutionId, institutionLogoUrl, onLogoUpdated }: AvatarUploadProps) => {
  ...
  const saveAvatarUrl = async (avatarUrl: string) => {
    ...
    try {
      if (isInstitution) {
        // Fallback: Currently the schema does not have a logo_url for institutions.
        // If an endpoint is added, it would be: await api.patch(`/admin/institutions/${institutionId}`, { logo_url: avatarUrl });
        onLogoUpdated?.(avatarUrl);
      } else {
        await api.patch("/profiles/me", { avatar_url: avatarUrl });
