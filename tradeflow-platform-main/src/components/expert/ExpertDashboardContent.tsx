...
const ExpertDashboardContent = () => {
  ...
  const { data: mySlots = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!user) return [];
      const { data } = await api.get("/appointments/my-slots");
      return data.slots;
    },
    enabled: !!user,
  });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      // Future: add an endpoint to check for unread l3_handoff notifications if needed
    }, 30000);
  ...
  const completeSession = useMutation({
    mutationFn: async ({ appointmentId, notes }: { appointmentId: string; notes: string }) => {
      await api.patch(`/appointments/${appointmentId}/complete`, { notes });
    },
    onSuccess: () => {
  ...
  const updateAppointmentStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      // Not strictly necessary if we use specific endpoints, but we can fall back to a generic update endpoint
      // Or we can use /appointments/:id/cancel if status === "cancelled"
      if (status === "cancelled") {
        await api.patch(`/appointments/${id}/cancel`);
      } else {
        toast.error("Status update not supported this way");
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["expert-appointments"] });
  ...
  const submitEscalation = useMutation({
    mutationFn: async () => {
      ...
      const { data } = await api.post(`/appointments/${escalationDialog.appointmentId}/escalate`, {
  ...
  const saveNote = useMutation({
    mutationFn: async ({ appointmentId, notes }: { appointmentId: string; notes: string }) => {
      await api.patch(`/appointments/${appointmentId}/complete`, { notes }); // Temporary workaround for saving notes without completing
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["expert-appointments"] });
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  // Update appointment
                  await api.patch(`/appointments/${rescheduleDialog.appointmentId}/reschedule`, {
