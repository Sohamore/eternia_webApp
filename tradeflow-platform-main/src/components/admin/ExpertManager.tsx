...
export default function ExpertManager() {
  ...
  const { data: experts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data.members || []).filter((m: any) => m.role === 'expert');
    },
  });

  // Availability is still read-only view; kept as empty until backend endpoint added
  const availability: any[] = [];

  const toggleActive = useMutation({
    mutationFn: async ({ id, is_active }: { id: string; is_active: boolean }) => {
      await api.patch(`/admin/members/${id}/${is_active ? 'deactivate' : 'activate'}`);
    },
  ...
  const toggleVerified = useMutation({
    mutationFn: async ({ id, is_verified }: { id: string; is_verified: boolean }) => {
      await api.patch(`/admin/members/${id}/verify`, { is_verified: !is_verified });
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin-experts"] }); toast.success("Updated"); },
    onError: (err: any) => toast.error(err.response?.data?.error || "Failed"),
  });
