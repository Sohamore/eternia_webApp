...
export default function MemberManager() {
  ...
  const { data: tempCredentials = [], isLoading: tempCredsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get(`/admin/temp-credentials?institution_id=${viewCredInstitution}`);
  ...
  const addMutation = useMutation({
    ...
    onSuccess: (profile) => {
      toast.success(`"${profile.username}" created as ${selectedRole}`);
  ...
  const bulkMutation = useMutation({
    mutationFn: async () => {
      const { data } = await api.post('/admin/temp-credentials/bulk', {
    ...
    onError: (err: any) => toast.error(err.response?.data?.error || "Failed to bulk create"),
  });
                            ...
                            {!m.is_verified && (
                              <Button
                                ...
                                onClick={async (e) => {
                                  ...
                                  try {
                                    await api.patch(`/admin/members/${m.id}/verify`, updateData);
...
export function ReferralCodesCard() {
  ...
  const { data: codes = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/admin/intern-referral-codes');
      return data.codes || [];
    },
  });
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const { data } = await api.post('/admin/intern-referral-codes', {});
      return data.code;
    },
    onSuccess: (code: any) => {
      toast.success(`Referral code created: ${code.code}`);
      navigator.clipboard.writeText(code.code);
      queryClient.invalidateQueries({ queryKey: ["admin-referral-codes"] });
    },
    onError: (err: any) => toast.error(err.response?.data?.error || "Failed to create code"),
  });
