  ...
  const { data: qrData, isLoading: qrLoading, error: qrError, refetch: regenerateQR } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        throw new Error(err.response?.data?.error || "Failed to generate QR");
      }
    },
  ...
  const { data: analytics } = useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        activeStudents: data.stats.totalStudents || 0, // Approx
        totalSessions: data.stats.totalSessions || 0,
        flaggedCount: data.stats.pendingEscalations || 0, // Approx
      };
    },
    enabled: !!profile?.institution_id,
  });
