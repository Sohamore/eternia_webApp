...
export default function RoleManager() {
  ...
  const { data: profiles = [], isLoading: isSearching } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get(`/admin/members?search=${searchTerm.trim()}`);
      return (data.members || []) as SelectedProfile[];
    },
  ...
  const assignMutation = useMutation({
    mutationFn: async () => {
      ...
      const { data } = await api.patch(`/admin/members/${selectedProfile.id}/role`, {
        role: selectedRole,
