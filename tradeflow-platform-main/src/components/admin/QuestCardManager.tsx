...
const QuestCardManager = () => {
  ...
  // Fetch all completions with answer (backend now includes user and quest details)
  const { data: allCompletions = [], isLoading: loadingCompletions } = useQuery({
    queryKey: ["admin-quest-completions"],
    queryFn: async () => {
      const { data } = await api.get('/quests/admin/completions');
  ...
  const updateCard = useMutation({
    mutationFn: async (id: string) => {
      await api.patch(`/quests/admin/${id}`, {
  ...
  const toggleActive = useMutation({
    mutationFn: async ({ id, is_active }: { id: string; is_active: boolean }) => {
      await api.patch(`/quests/admin/${id}`, { is_active: !is_active });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-quest-cards"] });
      queryClient.invalidateQueries({ queryKey: ["quest-cards"] });
              ...
              <Table>
                ...
                <TableBody>
                  {allCompletions.map((c: any) => (
                    <TableRow key={c.id}>
                      <TableCell className="font-medium text-xs">{c.user?.username || "Unknown"}</TableCell>
                      <TableCell className="text-xs max-w-[200px] truncate">{c.quest?.title || "Deleted"}</TableCell>
                      <TableCell className="text-xs max-w-[300px]">{c.answer || <span className="text-muted-foreground italic">No answer</span>}</TableCell>
                      <TableCell className="text-xs text-muted-foreground">{c.completed_at ? format(new Date(c.completed_at), "MMM d, h:mm a") : "Unknown Date"}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
