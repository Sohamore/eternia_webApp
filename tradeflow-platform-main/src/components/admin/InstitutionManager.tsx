...
const InstitutionManager = ({ onSelectInstitution }: InstitutionManagerProps = {}) => {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      const { data } = await api.post('/admin/institutions', {
        name: newName.trim(),
    ...
    onError: (err: any) => toast.error(err.response?.data?.error || "Failed to create institution"),
  });

  const toggleMutation = useMutation({
    mutationFn: async ({ id, is_active }: { id: string; is_active: boolean }) => {
      await api.patch(`/admin/members/institution/${id}/${!is_active ? 'activate' : 'deactivate'}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-institutions"] });
      toast.success("Institution updated");
    },
    onError: (err: any) => toast.error(err.response?.data?.error || "Failed to update institution"),
  });

  const deleteInstitutionMutation = useMutation({
    mutationFn: async (institutionId: string) => {
      const { data } = await api.delete(`/admin/institutions/${institutionId}`);
      return data;
    },
    onSuccess: (data) => {
    ...
    onError: (err: any) => toast.error(`Failed to delete institution: ${err.response?.data?.error || err.message}`),
  });

  const bulkMutation = useMutation({
    mutationFn: async () => {
      if (!bulkInstitution) throw new Error("No institution selected");
      const { data } = await api.post('/admin/members/bulk', {
        institution_id: bulkInstitution.id,
        count: parseInt(bulkCount),
        prefix: bulkPrefix.trim() || "user",
        role: bulkRole,
      });
      return data;
    },
    onSuccess: (data) => {
      toast.success(`${data.created_count} members created successfully!`);
      setBulkResults(data.members);
      queryClient.invalidateQueries({ queryKey: ["admin-members"] });
      queryClient.invalidateQueries({ queryKey: ["admin-inst-student-counts"] });
    },
    onError: (err: any) => toast.error(err.response?.data?.error || "Failed to bulk create members"),
  });
