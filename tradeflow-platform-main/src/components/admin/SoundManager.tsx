...
export default function SoundManager() {
  ...
  const { data: sounds = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/sound/admin/all');
      return data.sounds || [];
    },
  });

  // Note: File upload still requires storage — keeping URL manual input as fallback
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { data } = await api.post('/sound/upload', formData, {
      ...
      // Fallback: just use file name as placeholder, let admin enter URL manually
      toast.info("Auto-upload not configured. Please enter the file URL manually.");
      setUploadedFileName(file.name);
    } finally {
      setUploading(false);
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingId) {
        await api.patch(`/sound/${editingId}`, payload);
      } else {
        await api.post('/sound', payload);
      }
    },
    onSuccess: () => {
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="p-3 rounded-xl bg-card border border-border/50 space-y-2.5">
          ...
          <Input placeholder="File URL (e.g. https://...)" value={form.file_url} onChange={(e) => setForm((p) => ({ ...p, file_url: e.target.value }))} className="h-9" />
