...
const EscalationManager = () => {
  ...
  const createEscalation = useMutation({
    mutationFn: async () => {
      ...
      const { data } = await api.post('/admin/escalations', {
        justification,
      });
      return data;
    },
  ...
  const updateEscalation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      if (!user) throw new Error("Not authenticated");
      let endpoint = `/admin/escalations/${id}/approve`;
      if (status === 'rejected') endpoint = `/admin/escalations/${id}/reject`;
      // 'resolved' might need a separate endpoint or patch
