...
const TherapistDashboardContent = ({ isMobile }: { isMobile?: boolean }) => {
  ...
  const fetchQueue = useCallback(async () => {
    try {
      const { data } = await api.get("/blackbox/therapist/queue");
      if (data && data.queue) {
        setQueue(data.queue);
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    const checkActive = async () => {
      try {
        const { data } = await api.get("/blackbox/therapist/active");
        if (data && data.sessions && data.sessions.length > 0) {
          setActiveSession(data.sessions[0]);
          setActiveTab("session");
          if ((data.sessions[0] as any).room_id) {
            try {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        const { data } = await api.get("/blackbox/therapist/history");
        if (data && data.history) setHistory(data.history as unknown as EscalationRecord[]);
      } catch (e) {
  ...
  const acceptSession = async (sessionId: string) => {
    ...
    try {
      ...
      const { data } = await api.patch(`/blackbox/therapist/sessions/${sessionId}/accept`, { room_id: roomId });
  ...
  const endSession = async () => {
    ...
    try {
      await api.patch(`/blackbox/therapist/sessions/${activeSession.id}/notes`, { session_notes_encrypted: sessionNotes || null });
  ...
  const submitEscalation = async () => {
    ...
    const { data } = await api.patch(`/blackbox/therapist/sessions/${activeSession.id}/escalate`, {
      level,
      reason: escalationReason,
    });
                 ...
                 <MeetingProvider
                  ...
                  >
                    <MeetingView
                      ...
                      onJoined={async () => {
                        try {
                          await api.patch(`/blackbox/therapist/sessions/${activeSession.id}/join`);
                        } catch (e) {
                          console.error("Failed to update join time", e);
                        }
                      }}
