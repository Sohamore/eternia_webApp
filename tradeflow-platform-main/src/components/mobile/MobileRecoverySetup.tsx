...
const MobileRecoverySetup = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      await api.post("/profiles/recovery", { fragment_pairs: pairs, emoji_pattern: emojis });
      setIsComplete(true); toast.success("Recovery setup complete!");
    } catch (e: any) { toast.error(e.response?.data?.error || e.message); } finally { setIsSaving(false); }
  };
