...
const MobileExpertDashboard = () => {
  ...
  const createSlot = useMutation({
    mutationFn: async () => {
      ...
      await api.post("/appointments/slots", { start_time: s.toISOString(), end_time: e.toISOString() });
    },
  ...
  const deleteSlot = useMutation({
    mutationFn: async (slotId: string) => { await api.delete(`/appointments/slots/${slotId}`); },
