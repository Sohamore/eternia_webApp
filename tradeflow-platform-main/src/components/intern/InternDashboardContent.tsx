...
const InternDashboardContent = () => {
  ...
  const { data: trainingModules = [], isLoading: modulesLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/admin/training-modules');
      return data.modules || [];
    },
  });
  ...
  const submitEscalation = useMutation({
    mutationFn: async () => {
      ...
      if (!snippet) {
        try {
          const { data } = await api.get(`/peers/sessions/${escalationDialog.sessionId}/messages?limit=10`);
      ...
      const { data } = await api.patch(`/peers/sessions/${escalationDialog.sessionId}/flag`, {
        escalation_note: escalationReason,
        justification: snippet || escalationReason,
      });
      if (data?.contact) {
  ...
  const handleCompleteModule = async (mod: TrainingModule) => {
    if (mod.has_quiz && mod.quiz_questions.length > 0) {
      ...
      if (!allCorrect) {
        if (mod.day_number === 3 || mod.day_number === 6) {
          if (user) {
            await api.patch("/profiles/me", { training_status: "failed" });
          }
    ...
    if (user) {
      ...
      await api.patch(`/admin/members/${user.id}/verify`, {
        training_progress: newModules,
                        ...
                        {session.status === "active" && (
                          <>
                            ...
                            <Button size="sm" variant="outline" className="h-7 text-[11px] px-2.5" onClick={async () => {
                              await api.patch(`/peers/sessions/${session.id}/end`);
...
function ReferralCodeInput({ user, queryClient, refreshProfile }: { user: any; queryClient: any; refreshProfile: () => Promise<void> }) {
  ...
  const handleRedeem = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/profiles/me/redeem-referral', { code: code.trim().toUpperCase() });
      toast.success("Referral code applied! Dashboard unlocked.");
      await refreshProfile();
      queryClient.invalidateQueries({ queryKey: ["user-profile"] });
      window.location.reload();
    } catch (e: any) {
      toast.error(e.response?.data?.error || "Failed to redeem code");
    } finally {
      setLoading(false);
    }
  };
